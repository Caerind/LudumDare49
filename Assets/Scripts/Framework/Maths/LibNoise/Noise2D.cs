...
namespace LibNoise
{
    ...
    public class Noise2D : IDisposable
    {
        ...
        public Texture2D GetTexture(FilterMode filterMode = FilterMode.Point)
        {
            return GetTexture(GradientPresets.Grayscale, filterMode);
        }
        ...
        /// <returns>The created texture map.</returns>
        public Texture2D GetTexture(Gradient gradient, FilterMode filterMode = FilterMode.Point)
        {
        ...
        public Texture2D GetNormalMap(float intensity, FilterMode filterMode = FilterMode.Point)
        {
            var texture = new Texture2D(_width, _height);
            var pixels = new Color[_width * _height];
            for (var x = 0; x < _ucWidth; x++)
